...
def test_darknode_registration(
    ...
    assert (
        ren_token.balanceOf(darknode_registry_store)
        == registry_store_init_balance + C.POOL_BOND
    )
...
def test_darknode_registration_not_node_operator(
    node_operator, ren_pool, ren_token, user
):
    """
...
def test_darknode_registration_twice_diff_node_id(
    owner, node_operator, ren_pool, ren_token
):
    """
    ...
    # Attempt to register darknode twice should fail
    with reverts(""):
        ren_pool.registerDarknode(
            C.NODE_ID_2_HEX, C.PUBLIC_KEY_2, {"from": node_operator}
        )
