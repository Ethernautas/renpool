...
def test_withdraw_fullfulment(owner, user, ren_pool, ren_token):
    ...
    assert ren_pool.totalPooled() == POOL_TARGET
    assert ren_pool.withdrawRequests(0) # seeing if there is a withdraw request
    
    # Fullfilling
    ren_token.getFromFaucet({'from': user})
    assert ren_token.balanceOf(user, {'from': user}) == WITHDRAW_AMOUNT # Account 1 has 1k REN
    ...
    assert ren_pool.totalPooled() == POOL_TARGET # Pool still full